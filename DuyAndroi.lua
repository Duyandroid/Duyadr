...
shared.LoaderTitle = "Duy Android";
...
_G.FastAttackStrix_Mode = "God xeter Super Fast Attack";
spawn(function()
    while wait() do
        if _G.FastAttackStrix_Mode then
            pcall(function()
                if (_G.FastAttackStrix_Mode == "God Super Fast Attack") then
                    _G.Fast_Delay = 0000.1e-000,1;
                end
            end);
        end
    end
...
local v166 = v16.Sea:AddParagraph({
    Title = "Hoàn Thành Ải Draco V4 (ko có đâu)",
    Content = ""
});
